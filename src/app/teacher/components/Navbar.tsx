        ...
        <div className="relative">
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50 border border-[#e5e7eb]">
              <div className="px-4 py-2 text-gray-700 font-semibold border-b border-[#e5e7eb]">{user?.name || 'Giảng viên'}</div>
              <button
                className="w-full text-left px-4 py-2 hover:bg-[#f7f7f7] text-gray-700"
                onClick={() => { setShowDropdown(false); router.push('/teacher/profile'); }}
              >
                Thông tin cá nhân
              </button>
              <button
